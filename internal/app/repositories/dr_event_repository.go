...
type DREventRepository interface {
	...
	GetDREventsById(ctx context.Context, id string) ([]models.DREvents, error)
}
...
func (r *drEventRepository) GetDREventsById(ctx context.Context, id string) ([]models.DREvents, error) {
	query := `
        SELECT 
            dr.id AS id,
            dr.start_time,
